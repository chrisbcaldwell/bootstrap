...
// Chris Caldwell addition: moved sort to the Quantile method; no need if st err is target
...
// Chris Caldwell addition: StdErr

// StdErr returns the standard error of the resampled aggregate values.
// Resample must be called before this method or NaN is returned.
func (r *BasicResampler) StdErr() float64 {
	return stderr(r.sampleAggregates)
}
...
// Chris Caldwell addition: moved sort to Quantile method
...
// StdErr returns the standard error of the resampled aggregate values.
// Resample must be called before this method or NaN is returned.
func (r *PresampledResampler) StdErr() float64 {
	return stderr(r.sampleAggregates)
}
...
// stderr returns the standard deviation of vals
func stderr(vals []float64) float64 {
	length := len(vals)
	if length == 0 {
		return math.NaN()
	}
	return stat.StdDev(vals, nil) // nil weights, we want straight SD
}
